...
GRAVITY_ACCEL = 9.8  # gravity constant
...
class Dynamics(nn.Module):
    ...
    def forward(state, action):
        ...
        # Create Dynamic matricies
        # Note: Here gravity is used to change velocity which is the second element of the state vector
        # Normally, we would do x[1] = x[1] + gravity * delta_time
        # but this is not allowed in PyTorch since it overwrites one variable (x[1]) that is part of the computational graph to be differentiated.
        # Therefore, I define a tensor dx = [0., gravity * delta_time], and do x = x + dx. This is allowed...
        delta_state_gravity = t.tensor([0, 0.5* GRAVITY_ACCEL * FRAME_TIME, 0., 0., 0.])

        # Thrust
        # Note: Same reason as above. Need a 2-by-1 tensor.
        delta_dot_state =  t.tensor([[0., 0.], [math.cos(state[4]) * FRAME_TIME, 0.], [0., 0.], [math.sin(state[4]) * FRAME_TIME, 0.], [0., 1.]])
...
class Simulation(nn.Module):
    ...
    def initialize_state():
        state = [10, 0., 1, 0., .2617]  # TODO: need batch of initial states
        return t.tensor(state, requires_grad=True).float()

    def error(self, state):
        return state[0] ** 2 + state[1] ** 2 + state[2] ** 2 + state[3] ** 2 + math.sin(state[4])
